/* ============================================================================
   scroll-world â€” portable scroll-scrubbed camera-flight engine
   ----------------------------------------------------------------------------
     ...
     mountScrollWorld(document.getElementById('world'), {
       ...
       sections: [
         { id, label, still, stillMobile, clip, clipMobile, accent,
           scroll: 1.6,   // optional per-section override of diveScroll â€” more scroll
                          // distance = a slower, longer dwell in this scene
           linger: 0.5,   // optional 0..1 â€” remaps time so the camera settles mid-scene
                          // (exactly where the copy peaks) and moves quicker at the
                          // edges. 0 = linear (default). Keep â‰¤ 0.6; 1 = full pause.
           eyebrow, title, body, tags:[â€¦],
           cta:{ primary:{label,href}, secondary:{label,href} } }, // last section only
         â€¦
       ],
       connectors: [clipUrl, â€¦],          // length = sections.length - 1 (nulls allowed)
       connectorsMobile: [clipUrl, â€¦],    // optional lighter connectors for phones (same length)
   ...
   the rest of the phone handling below is always on)
     The engine is phone-aware out of the box: on a coarse-pointer / â‰¤860px viewport it
       - loads `clipMobile` / `connectorsMobile` when provided (encode these smaller +
         tighter-GOP â€” seek cost on a phone decoder is dominated by frames-from-keyframe,
       ...
       - keeps the still as a live poster until the clip actually paints its first frame,
         and primes each video (muted playâ†’pause) on first touch â€” this is what stops iOS
     ...
     Nothing here is required â€” a config with only `clip`/`connectors` still works on
...
function mountScrollWorld(container, config) {
  ...
  // the â‰¤860px query is read live via isMobile() so a desktop resize/DevTools toggle
  ...
  // ---- build the interleaved segment chain: dive0, conn0, dive1, â€¦ diveN-1 ----
  const SEGMENTS = [];
  ...
  // Per-section dwell: monotone remap of scrollâ†’time so the camera settles mid-scene
  ...
  function loadClip(s) {
    // Under prefers-reduced-motion we never load the clips at all â€” the stills stay up
    ...
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        ...
        // painted â€” on iOS a seeked-but-never-played muted video stays blank, so
  ...
  // first touch we prime every loaded clip (muted playâ†’pause) so the first seek is
...
function injectCSS() {
  ...
  // :root / .sw-root { --sw-bg / --sw-ink / --sw-accent â€¦ }) always win over
